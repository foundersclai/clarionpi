...
| backend/app/api | docs/module_contracts/app.api.view_models.md | status: implemented (M3-M6) — the only wire surface; auth/roles + gate envelope + per-gate VMs (incl. M5 plan/compliance/package) + drafting/package routes + M6 provenance routes (token→anchor lookup + app-served PHI-audited document blob) + wire token-scanner + closed submit schemas; SSE replay deferred |
