...
## As-landed scale (disclosure added 2026-07-18)

The sections above described this change as "focused hub-check tests" and understated its size.
S1 is complete, verified, and adds no runtime or schema behavior, but the honest scale is:

- `scripts/hub_check.py` grew 128 → 542 lines; `backend/tests/test_hub.py` is 851 lines with
  122 collected test cases.
- The tenant-key contract freezes 48 candidate-key shapes and 21 reference clauses maintained as
  three order-sensitive copies (ADR-0013 prose, checker constants, test expectations), so any
  future correction to a frozen shape costs a synchronized triple edit.
- Seven cases (`test_s1_adds_no_runtime_workshop_surface[*]`,
  `test_s1_tenant_contract_adds_no_schema_change[*]`) are point-in-time snapshots that would not
  detect a later PR adding a real route, model, or migration; removing them is optional cleanup
  that requires explicit human approval under the test-deletion boundary.
